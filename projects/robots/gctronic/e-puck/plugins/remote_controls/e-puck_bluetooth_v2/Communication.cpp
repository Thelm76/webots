...
bool Communication::initialize(const string &port) {
  try {
    ...
    // The firmware string should start with "Version 2." for e-puck-2
    static const char *expectedFirmware = "Version 2.";
    if (strncasecmp(&answer[2], expectedFirmware2, strlen(expectedFirmware2))) {
      cerr << "The firmware installed on the e-puck is " << &answer[2] << endl;
      cerr << "It is not compatible with the Webots remote-control protocol or the e-puck is not v2" << endl;
      cerr << "Please connect an e-puck2 or install a compatible firmware" << endl;
      free(answer);
      return false;
    }
    free(answer);
