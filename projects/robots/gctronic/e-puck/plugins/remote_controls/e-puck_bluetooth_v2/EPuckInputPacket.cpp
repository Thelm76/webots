...
void EPuckInputPacket::decode(int simulationTime, const EPuckOutputPacket &outputPacket) const {
  ...
  if (outputPacket.isMagnetometerRequested()) {
    double values[3];
    for (int i = 0; i < 3; i++) {
      values[i] = readFloatAt(currentPos);
      currentPos += sizeof(float);
    }

    TripleValuesSensor *magnetometer = DeviceManager::instance()->magnetometer();
    wbr_magnetometer_set_values(magnetometer->tag(), values);
    magnetometer->resetSensorRequested();
  }
  ...
  if (outputPacket.isTofSensorRequested()) {
    double value = readUShortAt(currentPos);
    currentPos += sizeof(unsigned short);
    ...
    tof->setLastRefreshTime(simulationTime);
  }
