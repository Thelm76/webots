...
void EPuckOutputPacket::apply(int simulationTime) {
  ...
  if (gyro->isSensorRequested()) {
    mGyroRequested = true;
    append(static_cast<char>(-'g'));
    mAnswerSize += 3 * sizeof(short);
  }

  // magnetometer management
  TripleValuesSensor *magnetometer = DeviceManager::instance()->magnetometer();
  if (magnetometer->isSensorRequested()) {
    mMagnetometerRequested = true;
    append(static_cast<char>(-'g'));    //TODO determine char
    mAnswerSize += 3 * sizeof(short);   //TODO determine length
  }
  ...
  if (tof->isSensorRequested()) {
    mTofSensorRequested = true;
    append(static_cast<char>(-'')); //TODO determine char
    mAnswerSize += sizeof(short);
  }
