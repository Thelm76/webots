...
float Packet::readFloatAt(int pos) const {
  if (pos + (int)sizeof(float) > mMaxSize)
    throw overflow_error("Packet max size overflow");

  for (int i = 0; i < 4; i++){
      bytesToFloat.byte[i] = static_cast<unsigned char>(mData[pos + i]);; //TODO or bytesToFloat.byte[4-i]
  }
  return bytesToFloat.floatVal;
}
